...
def compute_smallest_cost(cost_matrix, route, excess): 
    ...
    sol = np.zeros((K, len(route)))#, dtype=int)
...
def value(value_tables, coeff, types, sol, excess):
    ...
    for i in range(types):
        gain += value_tables[i][int(sol[i])]
        pol += coeff[i]*value_tables[i][int(sol[i])]
...
def get_solution_multiple_types(sol, max_sol, routes, types):
    ...
    solution = np.zeros((types, len(routes[0])))
    for i in range(types):
        solution[i] = get_solution_single_type(sol[i],max_sol[i],routes[i].length)
    return solution
...
def multi_types(cost_matrix, routes, coeff, excess):
    ...
    sol = np.zeros((types, K, len(routes[0])))#, dtype=int)
    values = np.zeros((types, K, len(routes[0])), dtype=float)
    max_omitted = np.zeros(types)
    ...
    for i in range(types):
        ...
        value_tables.append([values[i, j, len(routes[i]) -1] for j in range(int(max_omitted[i]))])
    
    solution = np.zeros(types)
    max_sol = np.zeros(types)
    max_val = 0
    for k in range(1, K):
...
if __name__ == '__main__':
    ...
    print(multi_types(env._env.costs_matrix, routes, coeff, info['excess_emission']))